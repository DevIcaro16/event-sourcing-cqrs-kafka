      ...
      kafkaConsumerLag.addCallback(async (result) => {
        try {
          ...
          for (const partition of offsets) {
            const groupTopicOffsets = groupOffsets.find(t => t.topic === this.topic)
            const groupPartition = groupTopicOffsets?.partitions.find(p => p.partition === partition.partition)
          ...
          console.warn(`[kafka:consumer] lag callback error (non-fatal): ${(err as Error).message}`)
        }
      })
